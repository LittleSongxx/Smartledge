  ...
  <div :class="cn('admin-shell min-h-dvh bg-admin-bg', desktopCollapsed && 'admin-shell--collapsed')">
    ...
    <aside class="admin-sidebar hidden min-h-dvh border-r border-admin-border bg-card lg:flex">
      ...
      <nav class="flex min-h-0 flex-1 flex-col overflow-y-auto px-2 py-3" aria-label="后台主导航">
        <section v-for="group in visibleNavGroups" :key="group.label" class="mb-4 last:mb-0">
      ...
      <div class="flex flex-none flex-col gap-2 border-t border-border p-2">
        <!--
          返回会话端：两端各自登录、各自持有 token，所以这里只是回到会话端页面，
          不涉及任何登录态搬运（会话端 token 本来就还在自己的存储里）。
        -->
        <Button
          as="a"
          variant="ghost"
          size="lg"
          :class="cn('w-full justify-start rounded-md', desktopCollapsed && 'justify-center px-0')"
          href="/chat"
          data-testid="admin-return-to-chat"
          title="返回会话端（用户端登录态独立保存）"
        >
          <ChatBubbleLeftRightIcon class="size-[18px] flex-none" aria-hidden="true" />
          <span v-if="!desktopCollapsed" class="truncate">返回会话端</span>
      ...
      <DrawerContent class="h-dvh max-h-dvh rounded-none" data-testid="admin-mobile-drawer">
        ...
        <aside class="flex min-h-0 flex-1 flex-col">
          ...
          <nav class="min-h-0 flex-1 overflow-y-auto px-3 py-4" aria-label="移动端后台导航">
            <section v-for="group in visibleNavGroups" :key="group.label" class="mb-5 last:mb-0">
          ...
          <div class="flex flex-none flex-col gap-2 border-t border-border p-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
            <Button as="a" variant="outline" size="lg" class="w-full rounded-md" href="/chat" data-testid="admin-return-to-chat-mobile">
              <ChatBubbleLeftRightIcon data-icon="inline-start" aria-hidden="true" />
              返回会话端
            </Button>
 ...
 * 菜单项按能力声明：
 *
 * - `permission` 是该页面主入口端点要求的权限（与控制器上的 `@RequiresPermission` 同名）。
 * - 菜单只按能力**收窄可见性**，不构成准入：真正的判定在后端 filter chain 与逐端点权限。
 *   因此"没有 perms 声明"的表现是少几项菜单，而不是多几个点了就 403 的入口。
 */
const navGroups = [
  {
    label: '总览',
    items: [
      { to: '/admin/dashboard', label: '运营总览', icon: HomeModernIcon, permission: 'document:read', exact: true },
      { to: '/admin/quality-overview', label: '知识运行全景', icon: ChartBarSquareIcon, permission: 'observe:read', exact: true }
    ]
  },
  {
    label: '知识资产',
    items: [
      { to: '/admin/documents', label: '文档接入', icon: ClipboardDocumentListIcon, permission: 'document:read' },
      { to: '/admin/knowledge-bases', label: '知识库管理', icon: CircleStackIcon, permission: 'kb:read', exact: true },
      { to: '/admin/document-acl', label: '文档授权', icon: ShieldCheckIcon, permission: 'document:acl:manage', exact: true }
    ]
  },
  {
    label: '路由与观测',
    items: [
      { to: '/admin/knowledge-route', label: '知识路由', icon: ShareIcon, permission: 'kb:read', exact: true },
      { to: '/admin/knowledge-route/traces', label: '路由追踪', icon: EyeIcon, permission: 'kb:read', exact: true },
      { to: '/admin/observability', label: '对话观测', icon: CommandLineIcon, permission: 'observe:read' }
    ]
  },
  {
    label: '系统设置',
    items: [
      { to: '/admin/members', label: '用户与角色', icon: UsersIcon, permission: 'user:manage', exact: true },
      { to: '/admin/settings/configuration', label: '参数配置', icon: Cog6ToothIcon, permission: 'config:read', exact: true }
    ]
  }
]
...
const visibleNavGroups = computed(() => navGroups
  .map((group) => ({
    label: group.label,
    items: group.items.filter((item) => hasAdminPermission(item.permission))
  }))
