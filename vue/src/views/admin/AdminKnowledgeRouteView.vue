  ...
  <section class="flex flex-col gap-4">
    <header class="glass-card glass-edge flex items-start justify-between gap-5 rounded-glass border px-[26px] py-6 max-[900px]:flex-col">
      <div class="w-full min-w-0">
        ...
        <div class="mt-3 flex min-w-0 items-center gap-3 max-sm:flex-col max-sm:items-stretch">
          <label class="shrink-0 text-sm font-medium text-foreground">知识库</label>
          <Select v-model="activeKnowledgeBaseId" :disabled="loading || actionLoading" @update:modelValue="handleKnowledgeBaseChange">
            <SelectTrigger aria-label="选择当前知识库" :title="activeKnowledgeBaseName" class="h-auto w-full min-w-0 max-w-full rounded-md border border-border bg-card px-4 py-2.5 text-sm font-semibold text-foreground sm:w-auto sm:min-w-[220px] sm:max-w-[28rem] *:data-[slot=select-value]:min-w-0">
              <SelectValue placeholder="请选择知识库" />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
                <SelectItem v-for="item in knowledgeBaseOptions" :key="item.id" :value="item.id">{{ item.baseName }}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
        </div>
    ...
    <section id="knowledge-route-panel-scope" v-show="activeTab === 'scope'" class="tab-content" role="tabpanel" aria-labelledby="knowledge-route-tab-scope" tabindex="0">
      <div class="glass-card glass-edge rounded-glass border p-[22px]">
        ...
        <div class="mt-4 grid max-h-[520px] gap-3 overflow-y-auto" style="grid-template-columns:repeat(auto-fill,minmax(280px,1fr))">
          <div v-for="item in filteredScopes" :key="item.scopeId"
    ...
    <section id="knowledge-route-panel-topic" v-show="activeTab === 'topic'" class="tab-content" role="tabpanel" aria-labelledby="knowledge-route-tab-topic" tabindex="0">
      <div class="glass-card glass-edge rounded-glass border p-[22px]">
          ...
          <Select v-model="activeScopeFilterValue">
            ...
            <SelectContent>
              <SelectGroup>
                ...
                <SelectItem v-for="item in scopes" :key="item.scopeId" :value="String(item.scopeId)">{{ item.scopeName }}</SelectItem>
              </SelectGroup>
            </SelectContent>
          </Select>
        ...
        <div class="mt-4 grid max-h-[520px] gap-3 overflow-y-auto" style="grid-template-columns:repeat(auto-fill,minmax(280px,1fr))">
          <div v-for="item in filteredTopics" :key="item.topicId"
            class="grid cursor-pointer gap-2 rounded-xl border p-3.5 transition-all"
    ...
    <section id="knowledge-route-panel-profile" v-show="activeTab === 'profile'" class="tab-content" role="tabpanel" aria-labelledby="knowledge-route-tab-profile" tabindex="0">
      <div class="glass-card glass-edge rounded-glass border p-[22px]">
        ...
        <div class="mt-3.5"><Input v-model="documentKeyword" aria-label="筛选文档画像" class="h-9 text-sm" placeholder="按文档名、原始文件名或知识库筛选文档" /></div>
          ...
          <template v-if="drawerType === 'scope'">
            ...
            <template v-if="drawerMode === 'edit'">
              <div class="mt-4 grid gap-2.5">
                <Select v-model="scopeForm.knowledgeBaseId" :disabled="Boolean(drawerTarget)">
                  <SelectTrigger aria-label="范围所属知识库" class="drawer-input h-auto">
                    <SelectValue placeholder="选择所属知识库" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectGroup>
                ...
                <Select v-model="scopeForm.parentScopeId">
                  ...
                  <SelectContent>
                    <SelectGroup>
                      ...
                      <SelectItem v-for="item in scopeParentOptions" :key="item.scopeId" :value="String(item.scopeId)">{{ item.scopeName }}</SelectItem>
                    </SelectGroup>
                  </SelectContent>
                </Select>
            ...
            <template v-if="drawerMode === 'edit'">
              <div class="mt-4 grid gap-2.5">
                <Select v-model="relationForm.topicId">
                  ...
                  <SelectContent>
                    <SelectGroup>
                      <SelectItem v-for="item in relationTopicOptions" :key="item.topicId" :value="String(item.topicId)">{{ item.topicName }}</SelectItem>
                    </SelectGroup>
                  </SelectContent>
                </Select>
...
const activeKnowledgeBaseName = computed(() => knowledgeBaseOptions.value.find((item) => sameId(item.id, activeKnowledgeBaseId.value))?.baseName || '请选择知识库')
...
const relations = computed(() => {
  ...
  return allRelations.value.filter((item) => {
    ...
    if (relationScopeFilterId.value && !sameId(topic?.scopeId, relationScopeFilterId.value)) return false
    if (!keyword) return true
...
function resetScopeForm() { Object.assign(scopeForm, { id: '', knowledgeBaseId: activeKnowledgeBaseId.value, scopeName: '', parentScopeId: '__none__', description: '', aliases: '', examples: '', sortOrder: '0', operatorId: OPERATOR_ID }); activeScopeId.value = '' }
...
async function loadAll() {
  ...
  try {
    ...
    if (relationScopeFilterId.value && !scopes.value.some((item) => sameId(item.scopeId, relationScopeFilterId.value))) relationScopeFilterId.value = ''
...
async function saveTopic() {
  await withAction(async () => {
    ...
    activeTopicId.value = data?.id || payload.id
