...
const failureDiagnostic = {
  ...
  code: 'GRAPH_RAG_INPUT_TOKEN_BUDGET_EXCEEDED',
  severity: 'ERROR',
  certainty: 'CONFIRMED',
  issues: [{
    ruleId: 'GRAPH_RAG_INPUT_TOKEN_BUDGET_EXCEEDED',
    message: '估算输入 4500 token，超过当轮有效预算 4000 token。',
    constraint: '输入 Token 必须不大于当轮有效输入预算。',
    suggestions: ['缩小来源单元或批次。', '核对供应商模型容量后再调整输入、输出和安全余量。'],
    parameters: [{
        ...
        <DataTableShell caption="文档处理状态示例">
          ...
          <tbody>
            <tr class="border-b border-border last:border-0">
              <td class="px-3 py-3 text-sm font-medium">检索核心流程完整验收说明文档</td>
...
{{ `{\n  "identity": "source-000128",\n  "citationEligible": true,\n  "stage": "PROMPT_RENDERED_SOURCE"\n}` }}
      ...
      <section class="lab-section" aria-labelledby="lab-failure-title">
        <div class="lab-section__heading">
          <h2 id="lab-failure-title">执行失败诊断</h2>
          <p>先解释原因、测量口径和动作，再保留原始详情供核对。</p>
        </div>
        <ExecutionFailureDiagnostic
          :diagnostic="failureDiagnostic"
          @navigate-config="diagnosticTarget = $event"
        />
        <p v-if="diagnosticTarget" class="mt-3 break-all font-mono text-caption text-muted-foreground">
          目标参数：{{ diagnosticTarget }}
        </p>
      </section>
