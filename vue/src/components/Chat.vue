...
.markdown-body {
  ...
  line-height: 1.8;
  /*
   * 用 break-word 而不是 anywhere：anywhere 会参与最小内容宽度计算，
   * 让引用令牌与紧随其后的中文句号之间出现断行点，句号被挤到下一行单独成行。
   * pretty 进一步抑制行尾孤字（含孤立的收尾标点）。
   */
...
.markdown-body :deep(.citation-token) {
  ...
  text-underline-offset: 3px;
  /*
   * button 默认是 inline-block，会形成独立行盒并制造断行点，
   * 使紧跟其后的中文句号被挤到下一行单独成行。
   * 改成 inline 让令牌与正文一起参与换行。
   */
  display: inline;
}
