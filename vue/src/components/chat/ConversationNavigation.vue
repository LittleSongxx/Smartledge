...
<template>
  ...
  >
    <header class="flex h-16 flex-none items-center gap-2 border-b border-border px-3">
      <div v-if="!collapsed" class="grid size-8 flex-none place-items-center rounded-md bg-primary text-xs font-bold text-primary-foreground" aria-hidden="true">NA</div>
  ...
  >
    <DrawerContent class="lg:hidden">
      <DrawerHeader class="flex-none border-b border-border pr-14 text-left">
        ...
        <!-- 窄屏看不到左侧品牌位（侧栏是 lg 以上才显示），版本信息落在这里补齐 -->
        <DrawerDescription>Smartledge · 选择已有会话，或开始一段新对话。</DrawerDescription>
