...
describe('F03 CSS and motion contracts', () => {
  it('keeps the cool slate page and blue accent colors token-driven', () => {
    const tokenCss = readFileSync(resolve(process.cwd(), 'src/assets/tailwind.css'), 'utf8')

    expect(tokenCss).toContain('--neutral-25: #f4f5f7;')
    expect(tokenCss).toContain('--neutral-50: #f4f5f7;')
    expect(tokenCss).toContain('--neutral-100: #f1f5f9;')
    expect(tokenCss).toContain('--neutral-200: #e2e8f0;')
    expect(tokenCss).toContain('--neutral-300: #cbd5e1;')
    expect(tokenCss).toContain('--neutral-600: #475569;')
    expect(tokenCss).toContain('--neutral-900: #111827;')
    expect(tokenCss).toContain('--brand-600: #2563eb;')
    expect(tokenCss).toContain('--admin-bg: #f4f5f7;')
    expect(tokenCss).toContain('--canvas-from: #f4f5f7;')
    expect(tokenCss).toContain('--logo-from: #2563eb;')
    expect(tokenCss).toContain('--logo-to: #1d4ed8;')
    expect(tokenCss).toContain('--logo-fg: #ffffff;')
    expect(tokenCss).toContain('--running-50: #e8f3fb;')
    expect(tokenCss).toContain('--running-200: #a6cfe8;')
    expect(tokenCss).toContain('--running-700: #115578;')
    expect(tokenCss).toContain('--route-mode-auto-bg: #e8f1fa;')
    expect(tokenCss).toContain('--route-mode-auto-fg: #2a578c;')
    expect(tokenCss).toContain('--route-mode-shadow-bg: #f3eef8;')
    expect(tokenCss).toContain('--route-mode-shadow-fg: #6a4f96;')
