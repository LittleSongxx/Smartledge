...
describe('F09 compatibility cleanup contracts', () => {
  ...
  it('removes the temporary chip token and knowledge-base forwarding wrapper', () => {
    const applicationSource = sourceFiles().map((path) => readFileSync(path, 'utf8')).join('\n')
    ...
    expect(source('src/views/admin/AdminKnowledgeBaseView.vue')).toContain('<ChildPageDialog')
  })
...
describe('F09 centered detail contracts', () => {
  it('keeps business, development, and evidence details out of right-side sheets', () => {
    const detailSource = [
      ...
      'src/components/admin/RagArtifactExplorer.vue',
      'src/components/admin/RagParseWorkbench.vue'
    ].map(source).join('\n')
