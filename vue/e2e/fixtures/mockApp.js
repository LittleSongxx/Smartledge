...
const fixtureSystemConfig = {
  ...
  categories: [{
    ...
    items: [{
      ...
      displayScale: 1,
      unit: '毫秒',
      relationHint: '必须满足：单通道超时 ≤ 子问题总超时。',
      relatedConfigKeys: ['ragRuntime.subQuestionTimeoutMs'],
      effectiveMode: 'NEW_CONVERSATION',
      effectiveModeLabel: '新会话生效'
    }, {
      configKey: 'ragRuntime.subQuestionTimeoutMs',
      label: '子问题总超时',
      description: '一个子问题的并行检索超过该时长后停止等待。',
      value: 60000,
      valueType: 'LONG',
      controlType: 'DURATION',
      minValue: 100,
      ...
      unit: '毫秒',
      relationHint: '必须满足：单通道超时 ≤ 子问题总超时。',
      relatedConfigKeys: ['ragRuntime.channelTimeoutMs'],
      effectiveMode: 'NEW_CONVERSATION',
      effectiveModeLabel: '新会话生效'
    }]
  }, {
...
let endpointEnvelopeOverrides = new Map()

export function setMockApiEnvelope(path, envelope) {
  endpointEnvelopeOverrides.set(path, envelope)
}
 ...
 * 用户端登录态夹具。
 *
 * 用户端与管理端登录态分开存储（B4）：`/chat` 需要用户端 token，`/admin/**` 需要管理端 token，
 * 因此测试夹具也必须分别设置。`installMockApp` 默认给一个有效的用户端登录态，
 * 让既有聊天页用例保持"已登录"的前提；需要验证未登录跳转的用例显式调用
 * `setChatAuthenticated(page, false)`。
 */
export async function setChatAuthenticated(page, authenticated) {
  // 注意：取消登录时写入空串而不是 removeItem —— 页面初始化脚本只在"键不存在"时播种默认登录态，
  // 写入空串既能表达"未登录"，又不会被下一次导航重新种上。
  await page.evaluate(({ tokenKey, userKey, token, enabled }) => {
    localStorage.setItem(tokenKey, enabled ? token : '')
    localStorage.setItem(userKey, enabled ? 'f09-visitor' : '')
  }, { tokenKey: CHAT_TOKEN_KEY, userKey: CHAT_USER_KEY, token: validToken(), enabled: authenticated })
}

export async function installMockApp(page) {
  endpointEnvelopeOverrides = new Map()
  // 用户端登录态用初始化脚本播种：它在每次导航的页面脚本之前执行，因此不依赖页面已加载，
  // 也不会在 beforeEach 阶段对 about:blank 调用 evaluate。
  //
  // 只在本次测试的**首次导航**播种（用 sessionStorage 标记）：否则用例显式登出后，
  // 下一次导航又会被重新种上登录态，导致"未登录跳转"的断言永远看不到。
  await page.addInitScript(({ tokenKey, userKey, token }) => {
    const marker = 'smartledge-e2e-chat-seeded'
    ...
    if (localStorage.getItem(tokenKey) === null) {
      ...
      localStorage.setItem(userKey, 'f09-visitor')
    }
  }, { tokenKey: CHAT_TOKEN_KEY, userKey: CHAT_USER_KEY, token: validToken() })
  await page.route('**/*', async (route) => {
    ...
    const envelope = endpointEnvelopeOverrides.get(url.pathname)
    const data = endpointData.has(url.pathname) ? endpointData.get(url.pathname) : null
    await route.fulfill({
      status: 200,
      contentType: 'application/json; charset=utf-8',
      body: JSON.stringify(envelope || { code: '0', message: 'ok', data })
    })
  })
}
