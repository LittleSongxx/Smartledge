...
import { fixtureDocument, installMockApp, setAdminAuthenticated, setChatAuthenticated, setMockApiEnvelope } from './fixtures/mockApp.js'
...
async function openRoute(page, route) {
  ...
  await setChatAuthenticated(page, true)
...
test('redirects, auth guard, and observation return context stay intact', async ({ page }) => {
  ...
  // B4：用户端独立登录。清掉用户端登录态后 /chat 必须落到用户登录页，且与后台登录态互不代偿。
  await setChatAuthenticated(page, false)
  await page.goto('/chat')
...
test('system configuration edits one typed value and exposes its before and after history', async ({ page }) => {
  ...
  const channelTimeoutRow = page.locator('[data-config-item="ragRuntime.channelTimeoutMs"]')
  const subQuestionTimeoutRow = page.locator('[data-config-item="ragRuntime.subQuestionTimeoutMs"]')
  await expect(channelTimeoutRow).toContainText('必须满足：单通道超时 ≤ 子问题总超时。')
  await channelTimeoutRow.getByRole('button', { name: '子问题总超时' }).click()
  await expect(subQuestionTimeoutRow).toBeFocused()
...
test('system configuration accepts exact integer GraphRAG budgets through native form submission', async ({ page }) => {
  const budgets = [
    ['documentBudgetMillis', 'GraphRAG 文档预算', 1200000],
    ['toolBudgetMillis', 'GraphRAG 工具预算', 100000],
    ['reserveMillis', 'GraphRAG 传输预留', 9000]
  ]
  const snapshot = {
    configVersion: 7, instanceStartVersion: 6, pendingRestartCount: 3,
    categories: [{
      groupKey: 'graphRag', groupLabel: 'GraphRAG',
      categoryKey: 'graphRagExecution', categoryLabel: 'GraphRAG 运行保护',
      items: budgets.map(([key, label, value]) => ({
        configKey: `graphRag.execution.${key}`, label, value: value + 1,
        effectiveValue: value + 1, valueType: 'LONG', controlType: 'DURATION',
        minValue: 1, maxValue: 3600000, step: 100, displayScale: 1, unit: '毫秒',
        effectiveMode: 'RESTART_REQUIRED', effectiveModeLabel: '重启应用后生效'
      }))
    }]
  }
  setMockApiEnvelope('/manage/config/current/query', { code: '0', data: snapshot })
  setMockApiEnvelope('/manage/config/item/update', { code: '0', data: snapshot })
  await openRoute(page, routes.find((route) => route.key === 'admin-system-config'))
  for (const [key, label, value] of budgets) {
    await page.locator(`[data-config-item="graphRag.execution.${key}"] [data-config-edit]`).click()
    const dialog = page.getByRole('dialog', { name: `编辑 ${label}` })
    await dialog.getByRole('spinbutton', { name: '新值' }).fill(String(value))
    await dialog.getByRole('textbox', { name: '修改说明' }).fill('整数毫秒回归')
    const request = page.waitForRequest((request) => request.url().endsWith('/manage/config/item/update'))
    await dialog.getByRole('button', { name: '保存修改' }).click()
    expect((await request).postDataJSON()).toMatchObject({ configKey: `graphRag.execution.${key}`, value })
    await expect(dialog).toBeHidden()
    await expect(page.getByRole('status').filter({ hasText: `${label}已更新` })).toContainText('等待重启 Java 后生效')
  }
})

test('system configuration shows saved and instance-effective restart values', async ({ page }) => {
  await page.setViewportSize({ width: 320, height: 568 })
  setMockApiEnvelope('/manage/config/current/query', {
    code: '0',
    message: 'ok',
    data: {
      configVersion: 7,
      instanceStartVersion: 6,
      ...
      categories: [{
        ...
        items: [{
          ...
          displayScale: 1,
          unit: '线程',
          effectiveMode: 'RESTART_REQUIRED',
  ...
  await openRoute(page, routes.find((route) => route.key === 'admin-system-config'))
  await expect(page.getByRole('status')).toContainText('1 项配置已保存，等待重启 Java 后生效')
  await expect(page.getByRole('status')).toContainText('当前实例启动于 v6')
  const row = page.locator('[data-config-item="graphRag.execution.workerThreads"]')
  await expect(row).toContainText('已保存 8 线程')
  await expect(row).toContainText('当前实例 6 线程')
  await expect(page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth)).resolves.toBe(0)
})

test('execution failure guidance preserves drafts and navigates from task logs to the exact parameter', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  const diagnostic = {
    schemaVersion: 'execution-failure-diagnostic.v1',
    code: 'RAG_CHANNEL_TIMEOUT_CONFLICT',
    ...
    certainty: 'CONFIRMED',
    issues: [{
      ruleId: 'RAG_CHANNEL_TIMEOUT_CONFLICT',
      message: '单通道超时不能大于子问题总超时。',
      constraint: '70000 ≤ 60000 毫秒不成立。',
      suggestions: ['降低单通道超时，或在确认整体延迟预算后提高子问题总超时。'],
      parameters: [{
        configKey: 'ragRuntime.channelTimeoutMs',
        label: '单通道超时',
        scope: 'SYSTEM',
        unit: '毫秒',
        effectiveValue: 70000,
        actualValue: null,
        measurementKind: 'UNKNOWN',
        effectiveModeLabel: '新会话生效',
        target: { type: 'SYSTEM_CONFIG', categoryKey: 'execution', configKey: 'ragRuntime.channelTimeoutMs' }
      }, {
        configKey: 'ragRuntime.subQuestionTimeoutMs',
        label: '子问题总超时',
        scope: 'SYSTEM',
        unit: '毫秒',
        effectiveValue: 60000,
        actualValue: null,
        measurementKind: 'UNKNOWN',
        effectiveModeLabel: '新会话生效',
        target: { type: 'SYSTEM_CONFIG', categoryKey: 'execution', configKey: 'ragRuntime.subQuestionTimeoutMs' }
      }]
    }]
  }

  setMockApiEnvelope('/manage/config/item/update', {
    code: '10054',
    message: '单通道超时不能大于子问题总超时。',
    data: diagnostic
  })
  ...
  const channelTimeoutRow = page.locator('[data-config-item="ragRuntime.channelTimeoutMs"]')
  await channelTimeoutRow.getByRole('button', { name: '编辑' }).click()
  const editDialog = page.getByRole('dialog', { name: '编辑 单通道超时' })
  await editDialog.getByRole('spinbutton', { name: '新值' }).fill('70000')
  await editDialog.getByRole('textbox', { name: '修改说明' }).fill('验证失败草稿保留')
  await editDialog.getByRole('button', { name: '保存修改' }).click()
  await expect(editDialog.locator('[data-failure-diagnostic]')).toContainText('70000 ≤ 60000 毫秒不成立')
  await expect(editDialog.locator('[data-failure-diagnostic]')).toContainText('实测未报告')
  await editDialog.locator('[data-diagnostic-parameter="ragRuntime.subQuestionTimeoutMs"]').click()
  await expect(page.locator('[data-config-item="ragRuntime.subQuestionTimeoutMs"]')).toBeFocused()

  await channelTimeoutRow.getByRole('button', { name: '编辑' }).click()
  await expect(page.getByRole('dialog', { name: '编辑 单通道超时' }).getByRole('spinbutton', { name: '新值' })).toHaveValue('70000')
  await expect(page.getByRole('dialog', { name: '编辑 单通道超时' }).getByRole('textbox', { name: '修改说明' })).toHaveValue('验证失败草稿保留')
  await page.getByRole('dialog', { name: '编辑 单通道超时' }).getByRole('button', { name: '取消' }).click()

  setMockApiEnvelope('/manage/config/item/update', { code: '0', message: 'ok', data: null })
  setMockApiEnvelope('/manage/document/detail/query', {
    code: '0',
    message: 'ok',
    data: {
      ...fixtureDocument,
      latestTaskId: 'task-failure-1',
      latestTaskType: '2',
      latestTaskTypeName: '索引构建',
      latestTaskStatus: '4',
      latestTaskStatusName: '执行失败'
    }
  })
  setMockApiEnvelope('/manage/document/task/log/query', {
    code: '0',
    message: 'ok',
    data: {
      logs: [{
        id: 'failure-log-1',
        stageTypeName: 'GraphRAG',
        eventTypeName: '失败',
        content: '构建失败，保留 FAILED 终态。',
        createTime: '2026-09-10T06:00:00Z',
        detailJson: JSON.stringify({ failureDiagnostic: diagnostic })
      }]
    }
  })
  await openRoute(page, routes.find((route) => route.key === 'admin-document-detail'))
  await page.getByRole('button', { name: /05 查看任务记录/ }).click()
  await page.getByRole('button', { name: '查看完整任务时间线' }).click()
  const taskDialog = page.getByRole('dialog', { name: '任务执行详情' })
  await expect(taskDialog.locator('[data-failure-diagnostic]')).toContainText('发生了什么')
  await expect(taskDialog.locator('pre')).toContainText('failureDiagnostic')
  await taskDialog.locator('[data-diagnostic-parameter="ragRuntime.channelTimeoutMs"]').click()
  await expect(page).toHaveURL(/\/admin\/settings\/configuration\?configKey=ragRuntime\.channelTimeoutMs$/)
  await expect(page.locator('[data-config-item="ragRuntime.channelTimeoutMs"]')).toBeFocused()
})
